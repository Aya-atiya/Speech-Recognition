...
def run_application(app_call):
    ...
    if "control" in app_call:
        try:
            texttospeech("Sure. Opening control panel Now")
            subprocess.Popen('C:\\Windows\\System32\\control.exe')
            app_open_reponse = "Success"
        except subprocess.CalledProcessError as e:
            app_open_reponse = e.output

    if "google" in app_call:
        try:
            texttospeech("Sure. Opening Google Now")
...
if __name__ == "__main__":
    ...
    APPLIST = ["calculator", "control", "google"]
    ...
    print("-- Speak Prompt --")
    texttospeech("Hello Aya. How are we doing today?")
    #time.sleep(3)
